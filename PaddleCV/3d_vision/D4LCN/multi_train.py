...
def train():
    ...
    nranks = fluid.dygraph.ParallelEnv().nranks
    epoch = (conf.max_iter / (d4lcn_reader.len / (conf.batch_size * nranks))) + 1
    ...
    with fluid.dygraph.guard(place):
        ...
        train_reader = fluid.io.batch(train_reader, batch_size=conf.batch_size)
        if args.use_data_parallel:
        ...
        for epo in range(int(epoch)):
            ...
            for batch_id, data in enumerate(train_reader()):
                ...
                iteration = epo * (d4lcn_reader.len/(conf.batch_size * nranks)) + batch_id
                ...
                if total_loss > 0:
                    ...
                    if (not 'batch_skip' in conf) or ((batch_id + 1) % conf.batch_skip) == 0:
                        optimizer._momentum = momentum_decay(iteration, conf.max_iter)   
                        optimizer.minimize(total_loss)
                        optimizer.clear_gradients()
                        cur_lr = optimizer.current_step_lr()
                        cur_mom = optimizer._momentum
                ...
                if iteration % conf.display == 0 and iteration > start_iter:
                    # log results
                     ...
                     print( "epoch %d | batch step %d | iter %d, lr: %.15f, mom: %10f,  batch cost: %.5f, loss %0.3f" % \
                            (epo, batch_id, iteration, cur_lr, cur_mom, train_batch_cost, total_loss.numpy()))
                ...
                if (iteration % conf.snapshot_iter == 0 and iteration > start_iter) and save_parameters:
                    ...
                    #    print("train_model.phase: ", train_model.phase)
