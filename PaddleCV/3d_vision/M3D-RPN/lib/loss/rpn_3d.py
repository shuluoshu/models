...
class RPN_3D_loss(fluid.dygraph.Layer):
    ...
    def forward(self, cls, prob, bbox_2d, bbox_3d, imobjs, feat_size):
        ...
        ctr_x_unsqueeze = fluid.layers.unsqueeze(input=to_variable(ctr_x), axes=0)
        ctr_y_unsqueeze = fluid.layers.unsqueeze(input=to_variable(ctr_y), axes=0)
        widths_unsqueeze = fluid.layers.unsqueeze(input=to_variable(widths), axes=0)
        heights_unsqueeze = fluid.layers.unsqueeze(input=to_variable(heights), axes=0)
        bbox_z3d_unsqueeze = fluid.layers.unsqueeze(input=src_anchors[:,4], axes=0)
        bbox_w3d_unsqueeze = fluid.layers.unsqueeze(input=src_anchors[:,5], axes=0)
        ...
        for bind in range(0, batch_size):
            ...
            if gts_val.shape[0] > 0 or gts_ign.shape[0] > 0:
                ...
                if fg_num > 0:
                    ...
                    bbox_x_bind_unsqueeze = fluid.layers.unsqueeze(bbox_x_bind, axes=1)
                    bbox_y_bind = bbox_y[bind, :]
                    bbox_y_bind_unsqueeze = fluid.layers.unsqueeze(bbox_y_bind, axes=1)
                    bbox_w_bind = bbox_w[bind, :]
                    bbox_w_bind_unsqueeze = fluid.layers.unsqueeze(bbox_w_bind, axes=1)
                    bbox_h_bind = bbox_h[bind, :]
                    bbox_h_bind_unsqueeze = fluid.layers.unsqueeze(bbox_h_bind, axes=1)
                    deltas_2d = fluid.layers.concat((bbox_x_bind_unsqueeze, bbox_y_bind_unsqueeze,
                                           bbox_w_bind_unsqueeze, bbox_h_bind_unsqueeze), axis=1)
                    ...
                    src_anchors.stop_gradient = True
                    if len(src_anchors.shape) == 1: 
                        src_anchors = fluid.layers.unsqueeze(input=src_anchors, axes=0)
                    ...
                    coords_2d = fluid.layers.concat((to_variable(bbox_x3d_dn_fg[np.newaxis,:] * bbox_z3d_dn_fg[np.newaxis,:]), to_variable(bbox_y3d_dn_fg[np.newaxis,:] * bbox_z3d_dn_fg[np.newaxis,:]), to_variable(bbox_z3d_dn_fg[np.newaxis,:])), axis=0)
                    coords_2d = fluid.layers.concat((coords_2d, to_variable(np.ones([1, coords_2d.shape[1]])).astype('float32')), axis=0)
                    ...
                    bbox_z3d_dn_tar = to_variable(bbox_z3d_dn_tar).astype('float32')
                    ...
                    bbox_ry3d_dn_tar = to_variable(bbox_ry3d_dn_tar).astype('float32')
                    ...
                    bbox_abs_z3d_var = fluid.layers.abs(bbox_z3d_dn_tar - bbox_z3d_dn_fg)
                    coords_abs_z[bind, fg_inds] = bbox_abs_z3d_var.numpy()
                    bbox_abs_ry3d_var = fluid.layers.abs(bbox_ry3d_dn_tar - bbox_ry3d_dn_fg)
        ...
        if self.cls_2d_lambda:
            ...
            if np.any(active):
                labels_active = fluid.layers.reshape(labels_active, shape=[-1,1])
                loss_cls = fluid.layers.softmax_with_cross_entropy(cls_active, labels_active, ignore_index=IGN_FLAG)
                labels_weight_active = fluid.layers.unsqueeze(labels_weight_active, axes=1)
                loss_cls = fluid.layers.elementwise_mul(loss_cls, labels_weight_active)
        ...
        if np.sum(bbox_weights) > 0:
            ...
            if self.bbox_3d_lambda:
                ...
                bbox_x3d_active = fluid.layers.gather(bbox_x3d, active_index_var)
                bbox_x3d_active = fluid.layers.unsqueeze(bbox_x3d_active, axes=0)

                bbox_y3d = fluid.layers.reshape(bbox_y3d, shape=[-1]) 
                bbox_y3d_active = fluid.layers.gather(bbox_y3d, active_index_var)
                bbox_y3d_active = fluid.layers.unsqueeze(bbox_y3d_active, axes=0)

                bbox_z3d = fluid.layers.reshape(bbox_z3d, shape=[-1]) 
                bbox_z3d_active = fluid.layers.gather(bbox_z3d, active_index_var)
                bbox_z3d_active = fluid.layers.unsqueeze(bbox_z3d_active, axes=0)

                bbox_w3d = fluid.layers.reshape(bbox_w3d, shape=[-1]) 
                bbox_w3d_active = fluid.layers.gather(bbox_w3d, active_index_var)
                bbox_w3d_active = fluid.layers.unsqueeze(bbox_w3d_active, axes=0)

                bbox_h3d = fluid.layers.reshape(bbox_h3d, shape=[-1]) 
                bbox_h3d_active = fluid.layers.gather(bbox_h3d, active_index_var)
                bbox_h3d_active = fluid.layers.unsqueeze(bbox_h3d_active, axes=0)

                bbox_l3d = fluid.layers.reshape(bbox_l3d, shape=[-1]) 
                bbox_l3d_active = fluid.layers.gather(bbox_l3d, active_index_var)
                bbox_l3d_active = fluid.layers.unsqueeze(bbox_l3d_active, axes=0)

                bbox_ry3d = fluid.layers.reshape(bbox_ry3d, shape=[-1]) 
                bbox_ry3d_active = fluid.layers.gather(bbox_ry3d, active_index_var)
                bbox_ry3d_active = fluid.layers.unsqueeze(bbox_ry3d_active, axes=0)

                loss_bbox_x3d = fluid.layers.smooth_l1(bbox_x3d_active.astype('float32'), bbox_x3d_tar_active.astype('float32'), outside_weight=bbox_weights_active.astype('float32'))
                loss_bbox_y3d = fluid.layers.smooth_l1(bbox_y3d_active.astype('float32'), bbox_y3d_tar_active.astype('float32'), outside_weight=bbox_weights_active.astype('float32'))
                loss_bbox_z3d = fluid.layers.smooth_l1(bbox_z3d_active.astype('float32'), bbox_z3d_tar_active.astype('float32'), outside_weight=bbox_weights_active.astype('float32'))
                loss_bbox_w3d = fluid.layers.smooth_l1(bbox_w3d_active.astype('float32'), bbox_w3d_tar_active.astype('float32'), outside_weight=bbox_weights_active.astype('float32'))
                loss_bbox_h3d = fluid.layers.smooth_l1(bbox_h3d_active.astype('float32'), bbox_h3d_tar_active.astype('float32'), outside_weight=bbox_weights_active.astype('float32'))
                loss_bbox_l3d = fluid.layers.smooth_l1(bbox_l3d_active.astype('float32'), bbox_l3d_tar_active.astype('float32'), outside_weight=bbox_weights_active.astype('float32'))
                loss_bbox_ry3d = fluid.layers.smooth_l1(bbox_ry3d_active.astype('float32'), bbox_ry3d_tar_active.astype('float32'), outside_weight=bbox_weights_active.astype('float32'))
            ...
            if self.bbox_3d_proj_lambda:
                ...
                loss_bbox_x3d_proj = fluid.layers.smooth_l1(bbox_x3d_proj_active.astype('float32'), bbox_x3d_proj_tar_active.astype('float32'), outside_weight=bbox_weights_active.astype('float32'))
                loss_bbox_y3d_proj = fluid.layers.smooth_l1(bbox_y3d_proj_active.astype('float32'), bbox_y3d_proj_tar_active.astype('float32'), outside_weight=bbox_weights_active.astype('float32'))
            ...
            coords_abs_z = fluid.layers.reshape(to_variable(coords_abs_z), shape=[-1])
            ...
            stats.append({'name': 'z', 'val': coords_abs_z_mean.numpy(), 'format': '{:0.2f}', 'group': 'misc'})

            coords_abs_ry = fluid.layers.reshape(to_variable(coords_abs_ry), shape=[-1])
